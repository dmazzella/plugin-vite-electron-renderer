 ...
 *
 * **Key Technique:**
 * We use `globalThis.require()` to load modules at runtime. This is
 * completely opaque to bundlers (esbuild, Rollup) since they cannot
 * statically analyze property access on globalThis. In Electron with
 * nodeIntegration enabled, `require` is injected as a global.
 */
...
// Load electron module using globalThis.require which is opaque to bundlers
const electron = typeof globalThis.require !== 'undefined'
 ...
 *
 * **The globalThis.require technique:**
 * We use `globalThis.require()` which is completely opaque to bundlers.
 * Neither esbuild nor Rollup can statically analyze property access on
 * globalThis, so the require() call is always preserved in the output.
 * At runtime in Electron (with nodeIntegration), globalThis.require is
 * the real Node.js require function.
 *
...
export function generateModuleSnippet(moduleName: string): string {
  ...
  // globalThis.require is opaque to bundlers - preserved in both dev and prod
  return `const _M_ = globalThis.require("${moduleName}");\n${exports}`;
}
...
export function generatePreBundledSnippet(
  ...
  return `const _M_ = globalThis.require("${requirePath}");\n${exports}`;
}
