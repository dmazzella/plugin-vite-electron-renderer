...
import type { Plugin as VitePlugin, Alias } from 'vite'
...
import { ensureDir, findNodeModules, log, normalizePath as normalizePathUtil, fileExists } from './utils.js'
import { generateElectronSnippet, generateBuiltinSnippet } from './snippets.js'
import { prebundleEsmModule, createCjsWrapper, createBuildArgs } from './bundler.js'
import { configureElectronBuild, addAliases, excludeFromOptimize } from './config.js'

// Re-export types
export type { ElectronRendererOptions, ModuleType, ModuleResolveConfig } from './types.js'
 ...
 * ```
 */
export default function electronRenderer(options: ElectronRendererOptions = {}): VitePlugin {
  const {
  ...
  return {
    ...
    async config(config, { command }) {
      ...
      const nodeModulesPath = findNodeModules(root) ?? path.posix.join(cwd, 'node_modules')
      ...
      const aliases: Alias[] = [
        createBuiltinAlias(cacheDir, moduleCache),
      ]
      
      // Add aliases for resolved modules (if any)
      if (resolveKeys.length > 0) {
        aliases.push(
          createResolveAlias(resolveKeys, moduleResolve, cacheDir, moduleCache, verbose)
        )
      }
...
function createResolveAlias(
  ...
  const resolvePattern = new RegExp(`^(${resolveKeys.map(escapeRegex).join('|')})$`)
  ...
  return {
    ...
    async customResolver(source, importer, resolveOptions) {
      ...
      if (!cachedPath) {
        const filename = path.posix.join(cacheDir, `${source.replace(/[@/]/g, '_')}.mjs`)
